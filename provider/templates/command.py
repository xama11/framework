from discord.ext import commands
from discord import app_commands

# Your imports

from application.containers.example import ExampleContainer
from application.containers.components.example import *
from application.decorators import cooldown

class Example(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.container = ExampleContainer()

    @app_commands.command(name="example", description="Example description")
    @cooldown.cooldown('example', seconds=100)
    async def example(self, interaction):
        await interaction.response.send_message(
            embed=self.container.embed(interaction), 
            view=ExampleButtons(self.container))
        
    # the example for @example is the name of the command function
    @example.error
    async def error(self, interaction, error):
        await interaction.response.send_message(error, ephemeral=True)
        
async def setup(bot):
    await bot.add_cog(Example(bot))
